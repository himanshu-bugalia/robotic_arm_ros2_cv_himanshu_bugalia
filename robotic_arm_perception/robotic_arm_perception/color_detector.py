...
class ColorDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        for color_id, (lower, upper) in color_ranges.items():
            ...
            for cnt in contours:
                if cv2.contourArea(cnt) > 1:  # Increased minimum area threshold
                    ...
                    Z = Camera_height - Box_height  # Assumed depth/distance
                    # Y = (cx_pix - self.cx) * Z / self.fx * -10
                    X0 = (cx_pix - self.cx) * Z / self.fx
                    Y0 = (cy_pix - self.cy) * Z / self.fy
                    ...
                    try:
                        ...
                        # self.get_logger().info(f"{color_id}: TF lookup succeeded")
                        ...
                        pt_cam = np.array([X, Y, Z_link, 1.0])
                        pt_base = T @ pt_cam
        ...
        # self.get_logger().info(f"HSV frame min={hsv.reshape(-1,3).min(axis=0)}, max={hsv.reshape(-1,3).max(axis=0)}")
